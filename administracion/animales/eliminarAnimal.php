<?php
    ...
    if(isset($_POST['eliminar'])){
        $tarjeta = $_POST['animal'];
        
        $resultadoFinal = $CRUD -> eliminarAnimal($tarjeta);
    }
?>
...
<body>
    ...
    <main class="container">
        <div id="columnaContenido">
            ...
            <div class="d-flex justify-content-between">
                ...
                <a class="btn w-100 m-1 btn-outline-mismoColor active" href="eliminarAnimal.php">
                    <span class="d-none d-md-inline">Eliminar</span>
                    <span class="d-inline d-md-none"><i class="fa fa-trash"></i></span>
                </a>
            </div>
            <div id="tarjetasZona" class="row p-2 pr-3">
                <input id="buscadorEliminarAnimales" type="text" class="form-control mb-3" placeholder="Buscar...">
                        ...
                        <tbody id="listaAnimales">
                            <?php
                                ...
                                foreach($resultado as $clave => $valor){
                                    echo "<tr>";
                                    echo '<form class="p-3" id="'.$valor["Tarjeta"].'" name="formularioEliminarAni" method="POST" action="#">';
                                    echo  "<td class='w-25'>
                                    <button type='button' id='opciones".$valor["Tarjeta"]."' onclick=mostrarOpcionesBorrado('".$valor["Tarjeta"]."') class='btn btn-link'>
                                        <img alt='eliminarAnimal' src='../../img/eliminar.png' width='20px'>
                                    </button>
                                    <button type='button' id='cancelar".$valor["Tarjeta"]."' onclick=mostrarCancelarBorrado('".$valor["Tarjeta"]."') class='btn botonMismoColor text-white m-1' style='display: none;'>Cancelar</button>
                                    <button type='submit' id='aceptar".$valor["Tarjeta"]."' name='eliminar' class='btn botonMismoColor text-white m-1 pr-3 pl-3' style='display: none;'>Aceptar</button>
                                    </td>";
                                    //echo  "<td><button type='submit' name='eliminar' class='btn btn-link'><img alt='eliminarAnimal' src='../../img/eliminar.png' width='20px'></button></td>";
                                    echo "<td><input type='hidden' value=$valor[Tarjeta] name='animal'>$valor[Tarjeta]</td>";
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="alert alert-success m-0 text-center">
                        ...
                        <span id="textoCorrecto"><b>¡Correcto!</b> Se ha eliminado un animal con exito</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        $(document).ready(function() {
            $(document).ready(function(){
                $("#buscadorEliminarAnimales").on("keyup", function() {
